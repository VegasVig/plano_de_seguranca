...
var PERGUNTAS_CONCORRENTE = [
  ...
  { q: 'Qual foi a rotatividade de vigilantes em postos semelhantes nos últimos doze meses?',
    p: 'Rotatividade alta significa vigilante que nunca aprende o posto. É o indicador que melhor prevê falha operacional.' },
...
function montarHTML(lev, fotos) {
  ...
  var logo = window.VEGAS_LOGO_PERSONALIZADA || window.VEGAS_LOGO.escura;
  ...
  P.push(cap(3, 'Índice de Maturidade em Segurança',
    '<div class="destaque" style="border-color:' + faixa.cor + '">' +
      '<div class="destaque-num" style="color:' + faixa.cor + '">' + indice.geral + '<span>/100</span></div>' +
      '<div class="destaque-txt"><strong>' + faixa.nome + '</strong><p>' + faixa.texto + '</p></div>' +
    '</div>' +
...
function pagina(corpo, titulo, logo, sub) {
  ...
  '</div>' +
  '<div class="doc">' +
    '<div class="cabecalho-fixo"><img src="' + logo + '" alt=""><span>' + esc(titulo) + (sub ? ' — ' + sub : '') + '</span></div>' +
    corpo +
  '</div></body></html>';
}
...
'body{margin:0;background:#8d8d8d;font-family:"Iowan Old Style","Palatino Linotype",Palatino,Georgia,serif;color:#191c21;font-size:10.5pt;line-height:1.55}',
'.doc{max-width:190mm;margin:0 auto;background:#fff;padding:18mm 16mm 20mm}',
'.barra-topo{position:sticky;top:0;z-index:9;background:#12151a;color:#e6e9ee;display:flex;gap:16px;align-items:center;justify-content:center;padding:10px 16px;font-family:system-ui,-apple-system,"Segoe UI",Roboto,sans-serif;font-size:13px;flex-wrap:wrap}',
'.barra-topo button{background:#e8a317;color:#12151a;border:0;border-radius:6px;padding:9px 16px;font-weight:700;font-size:13px;cursor:pointer}',
'h1{font-size:23pt;line-height:1.15;margin:6px 0 4px;font-weight:600;letter-spacing:-.01em}',
'h2{font-size:15pt;margin:0 0 14px;font-weight:600;display:flex;align-items:baseline;gap:10px;border-bottom:2px solid #191c21;padding-bottom:7px}',
'h2.sem-num{border-bottom-width:1px}',
'.cap-n{font-family:system-ui,sans-serif;font-size:10pt;font-weight:700;background:#191c21;color:#fff;width:22px;height:22px;border-radius:3px;display:inline-flex;align-items:center;justify-content:center;flex:0 0 auto}',
...
'.capa{page-break-after:always;text-align:left;padding-top:8mm}',
'.logo-capa{width:62mm;display:block;margin-bottom:22mm}',
'.capa-tipo{font-family:system-ui,sans-serif;font-size:9.5pt;letter-spacing:.14em;text-transform:uppercase;color:#6a727e;margin-bottom:4px}',
'.capa-local{font-size:12pt;color:#48505c;margin-bottom:16mm}',
'.capa-selo{display:flex;align-items:center;gap:16px;border-left:5px solid;padding:12px 18px;background:#f5f6f8;margin-bottom:14mm;max-width:100mm}',
'.selo-num{font-family:system-ui,sans-serif;font-size:38pt;font-weight:800;line-height:1}',
'.selo-txt{font-family:system-ui,sans-serif;font-size:10pt;line-height:1.4}',
'table{width:100%;border-collapse:collapse;margin:10px 0 14px;font-size:9.8pt}',
'.capa-dados th,.dados th{text-align:left;width:52mm;font-weight:600;color:#48505c;padding:5px 10px 5px 0;vertical-align:top;border-bottom:1px solid #e2e5ea}',
'.capa-dados td,.dados td{padding:5px 0;border-bottom:1px solid #e2e5ea}',
'.capa-rodape{margin-top:18mm;font-size:8.5pt;color:#6a727e;border-top:1px solid #d5d9df;padding-top:8px;max-width:120mm}',
'.cabecalho-fixo{display:none}',
'.sumario{font-size:11pt;line-height:2;padding-left:20px}',
'.nota{background:#f5f6f8;border-left:3px solid #b9c0ca;padding:9px 13px;font-size:9.5pt;color:#48505c;margin-top:14px}',
'.destaque{display:flex;gap:20px;align-items:center;border-left:6px solid;background:#f5f6f8;padding:16px 20px;margin:4px 0 18px}',
'.destaque-num{font-family:system-ui,sans-serif;font-size:44pt;font-weight:800;line-height:1}',
'.destaque-num span{font-size:14pt;font-weight:600;color:#8b929c}',
'.destaque-txt strong{font-size:13pt}.destaque-txt p{margin:3px 0 0;font-size:10pt}',
'.dims th{text-align:left;padding:7px 8px;border-bottom:1px solid #d5d9df}',
'.dims td{padding:7px 8px;border-bottom:1px solid #eceef1;vertical-align:middle}',
'.dims thead th{background:#191c21;color:#fff;font-size:9pt;font-weight:600}',
'.dims .num{font-family:system-ui,sans-serif;font-weight:700;width:14mm;text-align:right}',
'.dims .faixa{width:28mm;font-size:9pt;color:#48505c}',
'.barra{background:#e6e9ee;height:9px;border-radius:5px;overflow:hidden;min-width:34mm}',
'.barra-fill{height:100%;border-radius:5px}',
'.barra-vazia{font-size:7.5pt;color:#98a0ab;padding-left:4px;line-height:9px}',
'.escala td{padding:5px 8px;border-bottom:1px solid #eceef1;font-size:9.5pt}',
'.escala .e{color:#fff;font-family:system-ui,sans-serif;font-weight:700;width:20mm;text-align:center;font-size:9pt}',
...
'.dim-bloco em{color:#8a3b3b;font-style:normal}',
'.pill{font-family:system-ui,sans-serif;font-size:8.5pt;font-weight:700;color:#fff;padding:2px 8px;border-radius:10px;vertical-align:middle}',
'.ok{color:#2b6b3f}',
'.contadores{display:flex;gap:10px;margin-bottom:14px}',
'.cont{flex:1;background:#f5f6f8;padding:10px;text-align:center;font-family:system-ui,sans-serif;font-size:9pt;border-radius:4px}',
'.cont-n{font-size:24pt;font-weight:800;line-height:1.1}',
'.riscos th{background:#191c21;color:#fff;text-align:left;padding:6px 8px;font-size:8.8pt;font-weight:600}',
'.riscos td{padding:6px 8px;border-bottom:1px solid #eceef1;font-size:9.3pt}',
'.nivel{font-family:system-ui,sans-serif;font-size:8pt;font-weight:700;color:#fff;padding:2px 7px;border-radius:3px;white-space:nowrap}',
'.n-critico .nivel,.nivel.n-critico{background:#C6282C}',
'.n-alto .nivel,.nivel.n-alto{background:#E0651A}',
'.n-medio .nivel,.nivel.n-medio{background:#E8A317}',
'.n-baixo .nivel,.nivel.n-baixo{background:#7CB518}',
'.risco{border-left:4px solid #b9c0ca;padding:10px 0 10px 14px;margin-bottom:14px;break-inside:avoid}',
...
'.risco-meta{font-family:system-ui,sans-serif;font-size:8.5pt;color:#6a727e;margin-bottom:7px}',
'.quebra{page-break-before:always;padding-top:4mm}',
'.prazo{margin-bottom:18px;break-inside:avoid}',
'.prazo-tag{font-family:system-ui,sans-serif;font-size:8.5pt;color:#6a727e;margin:-4px 0 7px}',
'.acoes th{background:#f0f2f5;text-align:left;padding:6px 8px;font-size:8.8pt;border-bottom:1px solid #d5d9df}',
'.acoes td{padding:7px 8px;border-bottom:1px solid #eceef1;font-size:9.5pt;vertical-align:top}',
'.acoes .resp{width:32mm;border-left:1px solid #eceef1}',
'.acoes .check{width:16mm;border-left:1px solid #eceef1}',
'.pop{break-inside:avoid;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid #e6e9ee}',
'.pop h3{margin-top:0}',
'.pop-obj{font-size:10pt;color:#48505c}',
'.pop-passos{margin:6px 0;padding-left:20px;font-size:10pt}',
'.pop-passos li{margin-bottom:4px}',
'.pop-atencao{background:#fdf6e3;border-left:3px solid #e8a317;padding:8px 12px;font-size:9.5pt;margin-top:8px}',
'.dossie{padding-left:20px;font-size:10pt}',
'.dossie li{margin-bottom:6px;position:relative}',
'.dossie .box{display:inline-block;width:11px;height:11px;border:1px solid #98a0ab;margin-left:8px;vertical-align:baseline}',
...
'.foto img{width:100%;border:1px solid #d5d9df;display:block}',
'.foto-ausente{border:1px dashed #b9c0ca;padding:26px 10px;text-align:center;font-size:8.5pt;color:#8b929c}',
'.foto figcaption{font-size:8.5pt;color:#48505c;margin-top:4px;line-height:1.35}',
'.assinaturas{display:flex;gap:30px;margin-top:26mm;font-size:9.5pt}',
'.ass{flex:1;text-align:center}',
'.ass .linha{border-top:1px solid #191c21;margin-bottom:5px}',
'.ass small{color:#6a727e}',
'@media print{body{background:#fff}.doc{max-width:none;padding:0}.no-print{display:none!important}',
'.cabecalho-fixo{display:none}}'
].join('\n');
